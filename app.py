...
def main(page: ft.Page):
    ...
    # 🛠️ 오류 수정: 윈도우 크기 지정을 최신 표준 문법에 맞게 안전하게 수정했습니다.
    try:
...
if __name__ == "__main__":
    ...
    # 🛠️ 오류 수정: 웹 배포 환경(Render) 안정성을 높이기 위해 불필요한 view 옵션을 제거하고 기본값으로 기동합니다.
    ft.app(target=main, port=port, host="0.0.0.0", assets_dir=".")        with Image.open(current_path) as img_exif:
            exif_data = img_exif._getexif()
        if exif_data:
            from PIL.ExifTags import TAGS
            readable_exif = {TAGS.get(tag, tag): val for tag, val in exif_data.items()}
